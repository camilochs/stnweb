...
# Output: Network plots (pdf) saved in current folder. Tree layout used
#########################################################################
...
plotNet <-function(N, tit, nsizef, ewidthf, asize, ecurv, mylay, bleg = T) 
{
  ...
  if (bleg == T) {
    legend("bottomleft", legend.txt, pch = legend.shape, col = legend.col, horiz = T,
...
stn_decorate <- function(N)  {
  ...
  for (i in 1:length(alg_col)) {   
     # Assign color by algorithm order in vector - only colour nodes visited by a single alg.
...
rt = which(V(stnm)$Type == "start")  # determining the roots of the Tree
lt = layout_as_tree(stnm, root=rt, circular = F)  # Tree layout 

ofname <-  gsub('.{6}$', '', infile) # removes  (last 6characters) .RData from file to use as name
ofname = paste0("temp", "/",ofname,'-plot-tree.pdf')
...
plotNet(stnm, tit="Tree layout", nsizef=nf, ewidthf=ef, asize=0.16, ecurv=0.3, mylay=lt)
