...
alg_col    <-  c() #  orange, blue  green
alg_col_pt    <-  c() #  orange, blue  green
for (i in 4:length(args)) { 
  alg_col <- append(alg_col, args[i])
  alg_col_pt <- append(alg_col_pt, 16)
}
#stop(alg_col)
# Plot legend
...
legend.shape <- c(15,17, 16, alg_col_pt, 16)  # square and circles
...
plotNet <-function(N, tit, nsizef, ewidthf, asize, ecurv, mylay, bleg = T) 
{
  ...
  if (bleg == T) {
    legend("topleft", legend.txt, pch = legend.shape, col = legend.col, 
           cex = 0.7, pt.cex=1.4, bty = "n", pt.bg = c("white"))
  }  
}
...
stn_decorate <- function(N)  {
  ...
  for (i in 1:length(alg_col)) {    # Assign color by algorithm order in vector - only colour nodes visited by a single alg.
  ...
  # Color of edges 
  E(N)$color <- shared_col  # default c
  for (i in 1:length(alg_col)) { 
...
plotNet(stnm, tit="FR layout", nsizef=nf, ewidthf=ef, asize=size_arrow, ecurv=0.3, mylay=lfr)
# Slightly smaller nodes and edges for the KK layout as it spreads the components and makes the nodes
# closer to each other
plotNet(stnm, tit="KK Layout", nsizef=nf*0.8, ewidthf=ef*0.8, asize=size_arrow*0.8, ecurv=0.3, mylay=lkk)

# Plots of zoomed  -- Increased summed size and edges as the zoomed network has less nodes 
nf <- size_factor*1.5  
ef <- size_factor

plotNet(N = zoom, tit="Zoomed (top 25%) FR", nsizef=nf, ewidthf=ef, asize=size_arrow, ecurv=0.3, mylay=lzfr, bleg = T)
plotNet(N = zoom, tit="Zoomed (top 25%) KK", nsizef=nf*0.8, ewidthf=ef*0.8, asize=size_arrow, ecurv=0.3, mylay=lzkk, bleg = T)
